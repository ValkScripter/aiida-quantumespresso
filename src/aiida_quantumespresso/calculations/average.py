...
class AverageCalculation(CalcJob):
    ...
    def define(cls, spec):
        ...
        spec.input(
            ...
            required=True,
            help='Window size for the macroscopic average in Angstrom \
            (the code reads in Bohr, conversion is done internally)',
        )
        ...
        spec.output(
            ...
            help='The output data containing columns: z Ang, p(z) eV, m(z) eV. \
            Code outputs are in Bohr and Rydberg. Conversion is done internally.',
        )
    ...
    def prepare_for_submission(self, folder):  # pylint: disable=too-many-branches,too-many-statements
        ...
        if isinstance(source, orm.RemoteData):
            dirpath = os.path.join(source.get_remote_path(), self._DEFAULT_INPUT_DATA_FILE)
            remote_copy_list.append((source.computer.uuid, dirpath, self._DEFAULT_INPUT_DATA_FILE))
        elif isinstance(source, orm.FolderData):
            local_copy_list.append((source.uuid, self._DEFAULT_INPUT_DATA_FILE, self._DEFAULT_INPUT_DATA_FILE))
