...
class PdosWorkChain(ProtocolMixin, WorkChain):
    ...
    def _generate_dos_inputs(self):
        ...
        if dos_parameters.pop('align_to_fermi', False):
            dos_parameters['DOS']['Emin'] = dos_parameters.pop('Emin', None) + self.ctx.nscf_fermi
            dos_parameters['DOS']['Emax'] = dos_parameters.pop('Emax', None) + self.ctx.nscf_fermi
    ...
    def _generate_projwfc_inputs(self):
        ...
        if projwfc_parameters.pop('align_to_fermi', False):
            projwfc_parameters['PROJWFC']['Emin'] = projwfc_parameters.pop('Emin', None) + self.ctx.nscf_fermi
            projwfc_parameters['PROJWFC']['Emax'] = projwfc_parameters.pop('Emax', None) + self.ctx.nscf_fermi
